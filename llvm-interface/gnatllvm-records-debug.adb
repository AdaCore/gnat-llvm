...
package body GNATLLVM.Records.Debug is
   ...
   package Member_Vectors is new
     Ada.Containers.Vectors (Index_Type => Pos, Element_Type => Metadata_T);
   ...
   --  Convert a list of choices (the discrete choices selecting a variant
   --  part) to a word array.  The resulting word array has an even number
   --  of entries, with each pair holding the low and high bounds of a
   --  given choice.  Returns an empty array for 'others' or if the choice
   --  list is not recognized in some way.

   package Dw_Vector is new Ada.Containers.Vectors (Index_Type   => Nat,
                                                    Element_Type => uint64_t);
   --  Used when building a DWARF expression, which is just a vector of
   --  uint64_t; this particular representation is chosen because it is
   --  convenient to pass to LLVM.

   function Find_Discriminant_From_Index
     (Original_Type : Entity_Id; Index : Pos) return Entity_Id;
   --  Given the index of a discriminant (like Discriminant_Number) and a
   --  type, return the corresponding field.

   function Convert_One_Field
   ...
   --  update; if the field is a discriminant, then it is recorded in the
   --  map for later lookup.  If Artificial is True, the created field is
   --  marked artificial.  Returns the LLVM debug metadata, or
   --  No_Metadata_T is the field could not be converted.
   ...
   --  Convert a variant part to LLVM debug metadata, returning an array
   --  holding the LLVM debug metadata for all the relevant fields.

   function Convert_RI_Chain
     (M                : in out Discriminant_Map;
      Start            : Record_Info_Id;
      Original_Type    : Entity_Id;
      Is_Union         : Boolean;
      Toplevel_Members : in out Member_Vector;
      Outermost_Call   : Boolean) return Metadata_Array;
   --  Convert a chain of Record_Info_Ids to LLVM debug metadata, returning
   --  an array holding the metadata for the relevant fields.
   --  Toplevel_Members points to the vector of members for the outermost
   --  type being constructed; in some cases we may need to add a field
   --  there when processing a variant part.  This is only null for the
   --  outermost call to Convert_RI_Chain.
   ...
   --  A wrapper for Convert_RI_Chain that creates the outermost member
   --  vector and passes it in.
   ...
   is
      ...
      procedure Binop
        (Code : TCode; LHS : Node_Ref_Or_Val; RHS : Node_Ref_Or_Val) is
      begin
         Convert_It (LHS);
         Convert_It (RHS);
         ...
         case Code is
            ...
            when Trunc_Mod_Expr | Ceil_Mod_Expr | Floor_Mod_Expr =>
               ...
               --  This (erroneously) implements all forms of 'mod' the
               --  same way.  However note that DWARF does not clearly
               --  specify the definition of mod, so there are already
               --  debugger differences here.
      ...
      procedure Discriminant (Val : Node_Ref_Or_Val) is
         ...
         Bit_Offset_Dyn : constant Boolean              :=
           Bit_Offset < Uint_0;
         ...
         Size_In_Bytes  : constant ULL                  :=
           (Size_In_Bits + UBPU - 1) / UBPU;
         Pointer_Bits   : constant ULL                  :=
           ULL (Thin_Pointer_Size);
         Is_Unsigned    : constant Boolean              :=
           Is_Unsigned_Type (Etype (F));
         Rel_Bit_Offset : ULL;

      begin
         if Bit_Offset_Dyn then

            --  Dynamic offset to the discriminant, so a more complicated
            --  approach is needed.

            Convert_It (Bit_Offset);

            --  The offset in bits is on the stack.  It's convenient to
            --  compute the relative bit offset first and then keep it on
            --  the stack for later.
            ...
            begin
               ...
               Rel_Bit_Offset  := Offset_In_Bits - UBPU * Offset_In_Bytes;

               --  Compute the location of the discriminant

               Expression.Append (DW_OP_push_object_address);

               --  Frequently the discriminant is the first member, so
               --  avoid some extra work in this case.
         ...
         --  In the dynamic bit offset case, shift off the low bits.  This
         --  is done to avoid a more complicated calculation.
         ...
         if Is_Unsigned and then Rel_Bit_Offset = 0 then
            ...
            declare
               Left_Shift : constant ULL :=
                 Pointer_Bits - (Size_In_Bits + Rel_Bit_Offset);
            ...
            begin
               ...
               if Right_Shift /= 0 then
                  ...
                  Expression.Append ((if   Is_Unsigned then DW_OP_shr
                                      else DW_OP_shra));
               end if;
            end;
         end if;
   ...
   begin --  Start of processing for Convert_To_Dwarf_Expression
      ...
      --  If the outermost value is an ordinary constant, just return a
      --  constant.  This results in cleaner DWARF.
   ...
   begin
      return Iter : Entity_Id := Original_Record_Component (E) do
   ...
   begin
      ...
      then
         ...
         begin
            if Is_Bitfield (F) then
               ...
               MD := DI_Create_Member_Type
                 (No_Metadata_T, Name, File, Get_Physical_Line_Number (F_S),
                  UI_To_ULL (Esize (F)), Get_Type_Alignment (F_GT), Offset,
                  Mem_MD, Flags);
            end if;
         end;
         ...
         --  Unpatched LLVM.  There's nothing really sensible to do here

         return No_Metadata_T;
      else
         MD := Create_Member (DI_Builder, No_Metadata_T, Name, File,
                              Get_Physical_Line_Number (F_S), Size, Offset,
            ...
            if Is_Union then
               ...
               declare
                  ...
                  MD      : constant Metadata_T :=
                    Create_Variant_Member (DI_Builder, Fields, Choices);
   ...
   is
      ...
      Members_To_Update : constant access Member_Vector :=
        (if   Outermost_Call then Toplevel_Members'Access
   ...
   begin
      ...
      --  Convert the variant part, if any -- but only if compiled against
      --  an LLVM that supports variant parts with multiple members.

      if Types_Can_Have_Multiple_Variant_Members and then RI.Variants /= null
      then
         declare
         ...
         begin
            if Is_Union then
               ...
               if Ekind (Discrim) = E_Discriminant then
                  ...
                  --  If we already processed the discriminant, just reuse
                  --  its metadata here.  However, if the discriminant was
                  --  inherited, there will not be debuginfo for it.  In
                  --  this case, we emit an artificial discriminant into
                  --  this record, so that the variant part can refer to it.
                  ...
                  Members_To_Update.Append
                    (Create_Variant_Part (DI_Builder, Variant_MD, Parts));
               end if;
            end if;
         end;
      end if;
   ...
   begin
      ...
      --  A type might be self-referential.  For example, a record may have
      --  a member whose type refers back to the same record type.  To
      --  handle this case, we construct a empty composite type and record
      --  it; then later we update the members of the type.  Note that we
      --  pass a unique identifier here; that prevents LLVM from reusing an
      --  existing type in the case that this composite is nameless.
      ...
      begin
         --  At least in theory it seems that LLVM may replace the object
         --  entirely, so don't assume Result will be the same, and be sure
