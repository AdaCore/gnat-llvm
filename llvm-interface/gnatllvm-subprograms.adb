...
package body GNATLLVM.Subprograms is
   ...
   function Get_Param_Kind (Param : Formal_Kind_Id) return Param_Kind is
      ...
      Size         : constant ULL                        :=
        (if   Decls_Only or else Is_Nonnative_Type (GT)
         then ULL (Get_Bits_Per_Word) * 2 else Get_Type_Size (MD));
   ...
   function Create_Subprogram_Type
     (E : Subprogram_Type_Or_Kind_Id) return MD_Type
   is
      MD : MD_Type := Get_Subprogram_Type (E);

   begin
      if not Present (MD) then
         MD := Create_Subprogram_Type_Internal (E);
         Set_Subprogram_Type (E, MD);
      end if;
      ...
      while Present (Param_Ent) loop
         ...
         begin
            ...
            if PK_Is_In_Or_Ref (PK) then
               In_Arg_Types (J) :=
                 (if    PK = Foreign_By_Ref then  Pointer_Type (Type_Of (GT))
   ...
   begin
      ...
      while Present (Param) loop
         declare
            ...
            V      : constant GL_Value        :=
              (if    PK = Foreign_By_Component_Ref
               then  Get_Param (Func, Param_Num, Full_Component_GL_Type (GT),
                                No_MD_Type, R)
         ...
         begin
            ...
            --  integer to its actual type.
               ...
               declare
                  ...
                  MD   : constant MD_Type := Int_Ty (Nat (Size));
                  Val  : GL_Value;
   ...
   begin
      ...
      if Present (Alias (E)) then
         declare
            MD : constant MD_Type :=
              Pointer_Type (Create_Subprogram_Type (E));
   ...
   begin  -- Start of processing for Emit_Call
      ...
      if RK = Return_By_Parameter then
         Args (In_Idx) :=
           (if   Present (Outer_LHS) and then Is_Safe_From (Outer_LHS, N)
            then Convert_Ref (Get (Outer_LHS, Reference), Return_GT)
      ...
      --  level, or set up for our elab proc to do the initialization.
         ...
         declare
            ...
            function Int_To_Subp (V : GL_Value) return GL_Value is
              (GM (Int_To_Ptr (IR_Builder, +Addr, +Subp_MD, ""), GT,
                   Subp_MD, R, V))
         ...
         begin
            if Library_Level or else In_Elab_Proc then
               if No (V) then
                  V := G (Add_Global (Module, +Subp_MD, Get_Ext_Name (E)),
                          GT, Pointer_Type (Subp_MD), Ref (R));
                  Set_Value (E, V);
               end if;
               ...
               --  Otherwise, initialize this to null and add to elab proc
                  ...
                  Set_Initializer  (V, G (Const_Null (Subp_MD), GT,
                                          Type_For_Relationship (GT, R),
                                          R));
                  Add_To_Elab_Proc (N);
               end if;
   ...
   function Create_Subprogram (E : Subprogram_Kind_Id) return GL_Value is
      ...
      Subp_P_Type : constant MD_Type     := Pointer_Type (Subp_Type);
   ...
   begin
      if Global_Constructors.Last > 0 then
         ...
         Val := Const_Array  (Constructors, Any_Array_GL_Type, 1);
      ...
      if Global_Destructors.Last > 0 then
         ...
         Val := Const_Array  (Destructors, Any_Array_GL_Type, 1);
