		    ...
		    int Emit_C, unsigned char *success)
{
  ...
  *success = 0;
  ...
  Result->LongLongLongSize = Info->hasInt128Type() && !Emit_C ? 128 : Result->LongLongSize;
...
Get_Features (const char *TargetTriple, const char *Arch, const char *CPU)
{
  ...
  case Triple::aarch64: {
    ...
    auto const ArchKind = AArch64::parseArch(ArchSplit.first);

    if (ArchKind == AArch64::ArchKind::INVALID) {
      errs() << "warning: ignoring unsupported -march value " << Arch << "\n";
      return nullptr;
    }

    AArch64::getArchFeatures(ArchKind, Features);
